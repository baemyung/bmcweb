...
std::shared_ptr<boost::asio::ssl::context> getSslServerContext()
{
    ...
    BMCWEB_LOG_ERROR(
        "TEST: getSslServerContext = c.tlsStrict={}, hasWebuiRoute = {}",
        c.tlsStrict, forward_unauthorized::hasWebuiRoute());
    boost::asio::ssl::verify_mode mode = boost::asio::ssl::verify_none;
    if (c.tlsStrict)
    {
        BMCWEB_LOG_ERROR(
            "TEST: (c.tlsStrict==true) - Setting verify peer and fail if no peer cert");
    ...
    else if (!forward_unauthorized::hasWebuiRoute())
    {
        ...
        BMCWEB_LOG_ERROR(
            "TEST: (forward_unauthorized::hasWebuiRoute=={}) --- Setting verify peer only", forward_unauthorized::hasWebuiRoute());
    ...
    {
        BMCWEB_LOG_ERROR(
            "TEST: (forward_unauthorized::hasWebuiRoute==true) -- Setting verify NONE");
    }
