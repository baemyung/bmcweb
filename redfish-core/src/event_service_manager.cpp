...
EventServiceManager::EventServiceManager(boost::asio::io_context& iocIn) :
    ioc(iocIn)
{
...
EventServiceManager&
    EventServiceManager::getInstance(boost::asio::io_context* ioc)
{
    ...
    for (const auto& item : *obj)
    {
        ...
        else if (item.first == "Subscriptions")
        {
            for (const auto& elem : item.second)
            {
                ...
                persistent_data::UserSubscription& newSub = *newSubscription;
...
void EventServiceManager::updateSubscriptionData() const
{
    persistent_data::EventServiceStore::getInstance()
...
void EventServiceManager::setEventServiceConfig(
    const persistent_data::EventServiceConfig& cfg)
{
...
std::shared_ptr<Subscription>
    EventServiceManager::getSubscription(const std::string& id)
{
...
std::string EventServiceManager::addSubscriptionInternal(
...
std::string EventServiceManager::addSSESubscription(
    const std::shared_ptr<Subscription>& subValue, std::string_view lastEventId)
{
    ...
    if (!lastEventId.empty())
    {
        ...
        for (boost::circular_buffer<Event>::const_iterator event = lastEvent;
...
std::string EventServiceManager::addPushSubscription(
    const std::shared_ptr<Subscription>& subValue)
{
    std::string id = addSubscriptionInternal(subValue);
    subValue->deleter = [id]() {
...
bool EventServiceManager::deleteSubscription(const std::string& id)
{
    auto obj = subscriptionsMap.find(id);
    if (obj == subscriptionsMap.end())
    {
...
void EventServiceManager::deleteSseSubscription(
    const crow::sse_socket::Connection& thisConn)
{
...
size_t EventServiceManager::getNumberOfSubscriptions() const
{
    return subscriptionsMap.size();
}

size_t EventServiceManager::getNumberOfSSESubscriptions() const
{
    auto size = std::ranges::count_if(
        subscriptionsMap,
        [](const std::pair<std::string, std::shared_ptr<Subscription>>& entry) {
...
bool EventServiceManager::sendTestEventLog()
{
    for (const auto& it : subscriptionsMap)
    {
        std::shared_ptr<Subscription> entry = it.second;
...
void EventServiceManager::sendEvent(nlohmann::json::object_t eventMessage,
...
void EventServiceManager::resetRedfishFilePosition()
{
...
void EventServiceManager::cacheRedfishLogFile()
{
...
void EventServiceManager::stopEventLogMonitor()
{
    inotifyConn.reset();
}

void EventServiceManager::getReadingsForReport(sdbusplus::message_t& msg)
{
