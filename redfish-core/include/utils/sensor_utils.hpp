...
inline void checkSensorPurpose(
    const SensorServicePathMap& nextSensor, const SensorPurpose sensorPurpose,
 ...
 * the purpose for each of the sensors from <sensorListIn>.
 *
...
{
    BMCWEB_LOG_DEBUG("getSensorsByPurpose enter");
    ...
    for (const SensorServicePathMap& nextSensor : sensorListIn)
    {
        ...
        dbus::utility::getProperty<std::vector<std::string>>(
            ...
            "Purpose",
            [asyncResp, sensorPurpose, sensorMatches, callback, sensorPath,
             nextSensor, remainingSensorsToVist](
                const boost::system::error_code& ec,
                const std::vector<std::string>& purposeList) {
