                        ...
                        const crow::Response& resIn)
{
    ...
    req.reqPtr->erase(boost::beast::http::field::if_match);
                          ...
                          const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    if (req.reqPtr->method() != boost::beast::http::verb::patch &&
        req.reqPtr->method() != boost::beast::http::verb::post &&
        req.reqPtr->method() != boost::beast::http::verb::delete_)
    {
        messages::preconditionFailed(asyncResp->res);
        return false;
    }
